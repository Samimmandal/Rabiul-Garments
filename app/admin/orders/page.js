              ...
              <div key={order.id} className="bg-white border border-[#1b1b18]/15 p-5">
                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-700">{order.customer_email} · {order.customer_phone}</p>
                    ...
                    <div className="mt-4 grid sm:grid-cols-2 gap-3">
                      <div>
                        <label className="text-[10px] font-mono uppercase text-gray-500">Estimated Delivery</label>
                        <input
                          type="date"
                          defaultValue={order.estimated_delivery || ''}
                          onBlur={(e) => updateTracking(order.id, 'estimated_delivery', e.target.value || null)}
                          className="w-full border border-gray-300 px-2 py-1.5 text-sm outline-none"
                        />
                      </div>
                      <div>
                        <label className="text-[10px] font-mono uppercase text-gray-500">Tracking Note</label>
                        <input
                          type="text"
                          defaultValue={order.tracking_note || ''}
                          placeholder="e.g. Out for delivery"
                          onBlur={(e) => updateTracking(order.id, 'tracking_note', e.target.value || null)}
                          className="w-full border border-gray-300 px-2 py-1.5 text-sm outline-none"
                        />
                      </div>
                    </div>
