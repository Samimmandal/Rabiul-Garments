  ...
  return (
    <div className={`min-h-screen ${bg} ${text}`}>
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-6 py-12 md:py-16">
        <p className="text-[11px] font-mono uppercase tracking-[0.15em] text-[#2c6660] mb-3">
          Full catalog
        </p>
        ...
        {loading ? (
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px ${darkMode ? 'bg-[#f2ede1]/10' : 'bg-[#1b1b18]/15'} border ${border}`}>
            {products.map(p => {
              ...
              return (
                <Link key={p.id} href={`/product/${p.id}`} className={`${cardBg} group block`}>
                  <div className={`aspect-[3/4] ${darkMode ? 'bg-[#2a2a27]' : 'bg-[#e9e1d1]'} relative overflow-hidden`}>
                    ...
                    {discountPct && (
                      <span className="absolute top-3 right-3 bg-[#bd4632] text-white text-[10px] font-mono uppercase px-2 py-1">
                        {discountPct}% OFF
                      </span>
                    )}
                  </div>
                  <div className="p-4">
                    <p className={`text-[10px] font-mono uppercase ${muted} mb-1`}>
                      {p.category || 'Apparel'}
                    </p>
